...
static int ParseBlock( decoder_t *p_dec, const block_t *p_block )
{
    ...
    for( size_t i=0; i+1 < i_timings_count; i++ )
    {
        ...
        if( tt_time_Convert( &p_timings_array[i] ) < p_block->i_dts )
            continue;

        if( tt_time_Convert( &p_timings_array[i] ) > p_block->i_dts + p_block->i_length )
            break;
        ...
        {
            p_spu->i_start    = tt_time_Convert( &p_timings_array[i] );
            p_spu->i_stop     = tt_time_Convert( &p_timings_array[i+1] ) - 1;
