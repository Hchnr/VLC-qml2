...
static int Control( demux_t *p_demux, int query, va_list args )
{
    ...
    switch ( query )
    {
        ...
        case DEMUX_GET_LENGTH:
            *va_arg ( args, mtime_t * ) =  ( p_sys->frames_total * p_sys->frame_rate_denom / p_sys->frame_rate_num ) * CLOCK_FREQ;
            break;

        case DEMUX_GET_TIME:
            *va_arg( args, mtime_t * ) = __MAX(p_sys->i_pts, 0);
            break;
        ...
        case DEMUX_GET_PTS_DELAY:
            *va_arg( args, mtime_t * ) =
                ms_to_mtime( var_InheritInteger( p_demux, "file-caching" ) );
            return VLC_SUCCESS;
