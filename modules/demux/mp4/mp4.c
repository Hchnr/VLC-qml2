                       ...
                       unsigned i_max_preload )
{
    ...
    for( ; i_demux_max_nzdts >= i_current_nzdts; )
    {
        ...
        if( i_samplessize > 0 )
        {
            ...
            {
                es_out_SetPCR( p_demux->out, i_current_nzdts );
                p_sys->i_pcr = i_current_nzdts;
            }

            /* dts */
            p_block->i_dts = i_current_nzdts;
            /* pts */
...
static int FragSeekToTime( demux_t *p_demux, mtime_t i_nztime, bool b_accurate )
{
    ...
    if( b_iframesync && b_accurate )
        es_out_Control( p_demux->out, ES_OUT_SET_NEXT_DISPLAY_TIME, i_nztime );
    return VLC_SUCCESS;
}
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_GET_TIME:
            *va_arg( args, mtime_t * ) = p_sys->i_timescale > 0 ? p_sys->i_nztime : 0;
            return VLC_SUCCESS;
    ...
    if( p_sys->p_title )
        for( unsigned i = 0; i < BOXDATA(p_hmmt)->i_chapter_count; i++ )
        {
            ...
            if( s )
            {
                ...
                s->i_time_offset = ms_to_mtime( BOXDATA(p_hmmt)->pi_chapter_start[i] );
                TAB_APPEND( p_sys->p_title->i_seekpoint, p_sys->p_title->seekpoint, s );
            }
        }
}
                           ...
                           unsigned i_max_preload )
{
    ...
    for( uint32_t i = p_track->context.i_trun_sample; i < p_trun->i_sample_count; i++ )
    {
        ...
        msg_Dbg( p_demux, "tk(%i)=%"PRId64" mv=%"PRId64" pos=%"PRIu64, p_track->i_track_ID,
                 MP4_rescale( i_dts, p_track->i_timescale, CLOCK_FREQ ),
                 MP4_rescale( i_pts, p_track->i_timescale, CLOCK_FREQ ),
                 p_track->context.i_trun_sample_pos );
#endif
        if ( p_track->p_es )
        {
            p_block->i_dts = MP4_rescale( i_dts, p_track->i_timescale, CLOCK_FREQ );
            if( p_track->fmt.i_cat == VIDEO_ES && !( p_trun->i_flags & MP4_TRUN_SAMPLE_TIME_OFFSET ) )
                p_block->i_pts = VLC_TS_INVALID;
            else
                p_block->i_pts = MP4_rescale( i_pts, p_track->i_timescale, CLOCK_FREQ );
...
static int DemuxMoof( demux_t *p_demux )
{
    ...
    /* !important! Ensure clock is set before sending data */
    if( p_sys->i_pcr == VLC_TS_INVALID )
        es_out_SetPCR( p_demux->out, i_nztime );
...
static int DemuxFrag( demux_t *p_demux )
{
    ...
    if( i_status == VLC_DEMUXER_EOF )
    {
        ...
        if( i_demux_end != INT64_MIN )
            es_out_SetPCR( p_demux->out, i_demux_end );
    }
