...
static int Control( demux_t* p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_SET_TIME:
            ...
            {
                tt_time_t t = tt_time_Create( va_arg( args, mtime_t ) );
        ...
        case DEMUX_GET_LENGTH:
            if( p_sys->times.i_count )
            {
                ...
                *va_arg( args, mtime_t * ) = tt_time_Convert( &t );
                return VLC_SUCCESS;
            }
            break;
...
static int Demux( demux_t* p_demux )
{
           ...
           tt_time_Convert( &p_sys->times.p_array[p_sys->times.i_current] ) <= p_sys->i_next_demux_time )
    {
        ...
        {
            es_out_SetPCR( p_demux->out, i_playbacktime );
    ...
    if ( !p_sys->b_slave )
    {
        es_out_SetPCR( p_demux->out, p_sys->i_next_demux_time );
