...
bool virtual_segment_c::UpdateCurrentToChapter( demux_t & demux )
{
    ...
    {
        if ( p_current_vchapter != NULL && p_current_vchapter->ContainsTimestamp( sys.i_pts ))
            p_cur_vchapter = p_current_vchapter;
        else if (p_cur_vedition != NULL)
            p_cur_vchapter = p_cur_vedition->getChapterbyTimecode( sys.i_pts );
    }
    ...
    if ( p_cur_vchapter != NULL && p_current_vchapter != p_cur_vchapter )
    {
        ...
        if ( p_cur_vedition->b_ordered )
        {
            ...
            if ( !b_has_seeked )
            {
                ...
                sys.i_start_pts = p_cur_vchapter->i_mk_virtual_start_time;
                sys.i_mk_chapter_time = p_cur_vchapter->i_mk_virtual_start_time - p_cur_vchapter->segment.i_mk_start_time - ( ( p_cur_vchapter->p_chapter )? p_cur_vchapter->p_chapter->i_start_time : 0 );
            }
        }
                              ...
                              virtual_chapter_c *p_vchapter, bool b_precise )
{
    ...
    if ( p_vchapter != NULL )
    {
        ...
        if (CurrentEdition()->b_ordered)
            p_sys->i_mk_chapter_time = p_vchapter->i_mk_virtual_start_time - p_vchapter->segment.i_mk_start_time - ( ( p_vchapter->p_chapter )? p_vchapter->p_chapter->i_start_time : 0 );
