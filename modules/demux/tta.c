...
static int Demux( demux_t *p_demux )
{
    ...
    if( p_data == NULL )
        return VLC_DEMUXER_EOF;
    p_data->i_dts = p_data->i_pts = CLOCK_FREQ * p_sys->i_currentframe * TTA_FRAMETIME;
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_GET_LENGTH:
            *va_arg( args, mtime_t * ) = CLOCK_FREQ * p_sys->i_totalframes * TTA_FRAMETIME;
            return VLC_SUCCESS;

        case DEMUX_GET_TIME:
            *va_arg( args, mtime_t * ) = CLOCK_FREQ * p_sys->i_currentframe * TTA_FRAMETIME;
            return VLC_SUCCESS;
