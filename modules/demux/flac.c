...
typedef struct
{
    ...
    mtime_t i_length; /* Length from stream info */
    uint64_t i_data_pos;
...
static int Demux( demux_t *p_demux )
{
    ...
    if ( p_sys->p_current_block )
    {
        ...
        p_sys->p_current_block->i_dts = p_sys->b_start ? 0 : VLC_TS_INVALID;
    }
                            ...
                            p_sys->p_current_block ? &p_sys->p_current_block : NULL ) ) )
    {
        ...
        while( p_block_out )
        {
            ...
            if( unlikely(p_sys->i_pts == VLC_TS_INVALID) )
                es_out_SetPCR( p_demux->out, __MAX(p_block_out->i_dts - 1, 0) );
 ...
 * Control:
 *****************************************************************************/
static mtime_t ControlGetLength( demux_t *p_demux )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    const uint64_t i_size = stream_Size(p_demux->s) - p_sys->i_data_pos;
    mtime_t i_length = p_sys->i_length;
    int i;
...
static mtime_t ControlGetTime( demux_t *p_demux )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    return p_sys->i_pts;
}
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    {
        mtime_t *pi64 = va_arg( args, mtime_t * );
    ...
    else if( i_query == DEMUX_SET_POSITION )
    {
        ...
        mtime_t i_length = ControlGetLength( p_demux );
        int i_ret;
        if( i_length > 0 )
        {
    ...
    else if( i_query == DEMUX_GET_POSITION )
    {
        ...
        if( i_length > 0 )
        {
            mtime_t current = ControlGetTime(p_demux);
            if( current <= i_length )
            {
