...
static void StreamParserCueDoneHandler( void *priv, webvtt_cue_t *p_cue )
{
    ...
    if( p_cue->psz_text )
    {
        ...
        if( p_block )
        {
            ...
            {
                es_out_SetPCR( p_demux->out, p_cue->i_start );
                p_sys->b_first_time = false;
            }
            p_sys->i_next_demux_time = p_cue->i_start;
            p_block->i_dts =
                    p_block->i_pts = p_cue->i_start;
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_SET_NEXT_DEMUX_TIME:
            p_sys->b_slave = true;
            p_sys->i_next_demux_time = va_arg( args, int64_t );
            return VLC_SUCCESS;
...
static int Demux( demux_t *p_demux )
{
           ...
           p_sys->index.p_array[p_sys->index.i_current].time <= i_barrier )
    {
        ...
        if( p_block )
        {
            ...
            p_block->i_dts = p_block->i_pts = i_start_time;
    ...
    if ( !p_sys->b_slave )
    {
        es_out_SetPCR( p_demux->out, i_barrier );
        p_sys->i_next_demux_time += 1*CLOCK_FREQ;
    }
