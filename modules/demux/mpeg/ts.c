...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    case DEMUX_SET_TIME:
           ...
           !SeekToTime( p_demux, p_pmt, p_pmt->pcr.i_first + TO_SCALE(i64) ) )
        {
            ReadyQueuesPostSeek( p_demux );
            es_out_Control( p_demux->out, ES_OUT_SET_NEXT_DISPLAY_TIME,
                            FROM_SCALE(p_pmt->pcr.i_first) + i64 );
            return VLC_SUCCESS;
        }
        break;
    ...
    case DEMUX_GET_TIME:
        if( p_sys->b_access_control )
        {
            ...
            {
                *va_arg( args, mtime_t * ) = i_event_start * CLOCK_FREQ;
                return VLC_SUCCESS;
            }
        }
        ...
        if( p_pmt && p_pmt->pcr.i_current > -1 && p_pmt->pcr.i_first > -1 )
        {
            ...
            *va_arg( args, mtime_t * ) = FROM_SCALE(i_pcr - p_pmt->pcr.i_first);
            return VLC_SUCCESS;
        }
        break;
    ...
    while( (i_head_pos + p_sys->i_packet_size) <= i_tail_pos && !b_found )
    {
        ...
        while( i_pos < i_tail_pos )
        {
            ...
            if( i_pcr != -1 )
            {
                ...
                else if( i_diff < TO_SCALE(CLOCK_FREQ / 2) ) // 500ms
                    b_found = true;
                else
                    i_head_pos = i_pos;
                break;
