...
static void aout_DecSilence (audio_output_t *aout, mtime_t length, mtime_t pts)
{
    ...
    aout->play(aout, block,
               vlc_clock_ConvertToSystem(owner->sync.clock, mdate(), pts));
}

static mtime_t aout_DecSynchronize(audio_output_t *aout, mtime_t system_now,
                                   block_t *block)
{
    /**
        ...
        if (owner->sync.is_first)
        {
            mtime_t system_pts =
                vlc_clock_ConvertToSystem(owner->sync.clock, system_now + delay,
                                          block->i_pts);
            mtime_t silence_delay = system_pts - system_now;
            if (silence_delay > 0)
    ...
    mtime_t system_pts = vlc_clock_ConvertToSystem(owner->sync.clock, system_now,
                                                   block->i_pts);
                          ...
                          mtime_t audio_ts)
{
    ...
    mtime_t drift = -vlc_clock_Update(owner->sync.clock, system_ts, audio_ts,
                                      owner->sync.rate);
...
int aout_DecPlay(audio_output_t *aout, block_t *block)
{
    ...
    if (owner->sync.request_delay != owner->sync.delay)
    {
        ...
        mtime_t delta = vlc_clock_SetDelay(owner->sync.clock, owner->sync.delay);
        if (delta > 0)
        {
    ...
    /* Drift correction */
    mtime_t system_pts = aout_DecSynchronize(aout, mdate(), block);
...
void aout_DecFlush (audio_output_t *aout, bool wait)
{
    ...
    if (owner->mixer_format.i_format)
    {
        if (wait)
        {
            ...
            if (block)
            {
                mtime_t date = vlc_clock_ConvertToSystem(owner->sync.clock,
                                                         mdate(), block->i_pts);
