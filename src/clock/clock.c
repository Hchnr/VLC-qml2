                                       ...
                                       mtime_t pts, float rate)
{
    ...
    if (main_clock->offset != VLC_TS_INVALID && pts != main_clock->last.stream)
    {
        ...
        //printf("%lld - %lld\n", (system_now - main_clock->last.system), (pts - main_clock->last.stream));
        ...
        //printf("%f -> %f  (%f)\n", instant_coeff, main_clock->coeff, rate);
    }
    else
        main_clock->wait_sync_ref =
...
fprintf(stderr, "vlc_clock_master_set_delay: %" PRId64 "\n", delay);
    ...
    {
        if (main_clock->reset_date == VLC_TS_INVALID)
            main_clock->reset_date = mdate() + delta;
        else
...
static mtime_t vlc_clock_master_to_system_locked(vlc_clock_t * clock, mtime_t now,
                                                 mtime_t pts)
{
    vlc_clock_main_t * main_clock = clock->owner;
    assert(main_clock->pause_date == VLC_TS_INVALID);
